...
use crate::types::{TypeId, TypeGroup, TypeOrPlaceholder};
...
pub fn mir_update(
    ...
    for (_, mut root_node) in roots.iter_mut::<RootNode>() {
        ...
        for handle in constants {
            ...
            let new_node = Node::new(Kind::Application, Handle::nil(), iter::once(Some(new_node)).chain(iter::once(Some(kind))).chain(iter::once(Some(typ))).chain(iter::once(Some(constant))));
            let h = new_node.id();
            lazy.insert(h, new_node);
            let new_node = h;
            ...
            let assignment = Node::new(Kind::Binding, Handle::nil(), iter::once(Some(binding)).chain(iter::once(None)).chain(iter::once(Some(new_node))));
            ...
            interpolate.clone_from(Node::new(Kind::Application, Handle::nil(), iter::once(Some(rid)).chain(iter::once(Some(var)))));
        }
        ...
        let quote = Node::new(Kind::Application, Handle::nil(), iter::once(Some(qqid)).chain(iter::once(Some(root))));
        let qid = quote.id();
        ...
        let format = Node::new(Kind::Application, Handle::nil(), iter::once(Some(faid)).chain(iter::once(Some(qid))).chain(replacements.into_iter().map(Some)));
        let fid = format.id();
